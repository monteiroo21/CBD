...
### 26. Encontre o restaurante com a maior latitude na coleção.
```
db.restaurants.find({}, { _id: 0, nome: 1, 'address.coord': 1}).sort({ 'address.coord.0' : -1}).limit(1)
```
...
### 27. Liste os restaurantes que têm pelo menos dez avaliações feitas.
```
db.restaurants.aggregate([ { $match: { $expr: { $gte: [ { $size: "$grades" }, 8 ] } } }, { $project: { _id: 0, nome: 1, grades: 1 } } ])
```
...
### 28. Liste para cada gastronomia o número de restaurantes na localidade Bronx, em ordem decrescente de número de restaurantes
```
db.restaurants.aggregate([ { $match: { localidade: "Bronx" } }, { $group: { _id: "$gastronomia", numRestaurants: { $sum: 1 } } }, { $sort: { numRestaurants: -1 } } ])
```
...
### 29. Encontre os restaurantes com gastronomia "Delicatessen" ou "American", estão localizados em "Brooklyn" ou "Queens", e possuem pelo menos uma avaliação com score acima de 50.
```
db.restaurants.find({ $and: [{ $or: [{ gastronomia: 'Delicatessen' }, { gastronomia: 'American' } ]}, { $or: [{ localidade: 'Brooklyn' }, { localidade: 'Queens' } ]}, { 'grades.score': { $gt: 50 } }] }, { _id: 0, nome: 1, gastronomia: 1, 'grades.score': 1, localidade: 1 })
```
...
### 30. Liste os restaurantes que receberam mais de 5 avaliações e que possuem pelo menos uma avaliação com score superior a 90.
```
db.restaurants.aggregate([ { $match: { "grades.score": { $gt: 90 }, $expr: { $gt: [ { $size: "$grades" }, 5 ] } } }, { $project: { _id: 0, nome: 1, localidade: 1, gastronomia: 1, totalGrades: { $size: "$grades" }, grades: 1 } } ])
```
